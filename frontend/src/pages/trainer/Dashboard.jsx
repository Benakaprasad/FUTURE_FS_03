            ...
            <div style={ts.profileInfo}>
              ...
              <div style={ts.profileMeta}>
                ...
                {profile?.is_active !== undefined && (
                  <span style={{ ...ts.profileMetaItem, color: profile.is_active ? "#22C55E" : "#FF1A1A" }}>
                    {profile.is_active ? "● Active" : "● Inactive"}
                  </span>
                )}
              </div>
            </div>
            <Link className="trainer-schedule-btn" to="/trainer/schedule" style={ts.scheduleBtn}>
              📅 View Schedule
            </Link>
          </div>
          ...
          {expiringMembers.length > 0 && (
            <div className="trainer-alert" style={ts.alertBar}>
              ...
              <span style={{ flex: 1, fontSize: "0.9rem", color: "rgba(255,255,255,0.7)" }}>
                <strong>{expiringMembers.length} members</strong> have memberships expiring in the next 7 days.
              </span>
...
const ts = {
  ...
  scheduleBtn:    { padding: "10px 18px", marginLeft: "auto", background: "rgba(0,194,255,0.1)", border: "1px solid rgba(0,194,255,0.25)", borderRadius: "8px", color: "#00C2FF", textDecoration: "none", fontWeight: 700, fontSize: "13px", transition: "all 0.2s", flexShrink: 0 },
