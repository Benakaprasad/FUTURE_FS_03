...
export default function AdminLeads() {
  ...
  const openConvert = (lead) => {
    setConvertForm({
      membership_type: "monthly",
      start_date: new Date().toISOString().split("T")[0],
      end_date: "", amount_paid: "", payment_status: "pending", admission_notes: "",
    });
  ...
  const cols = [
      ...
      render: (r) => (
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <div style={{
            ...
            background: r.source === "website" ? "rgba(0,194,255,0.15)" : "rgba(255,26,26,0.1)",
            color: r.source === "website" ? "#00C2FF" : "#FF1A1A",
    ...
    { key: "phone", label: "Phone", render: (r) => r.phone || <span style={{ color: "rgba(255,255,255,0.2)" }}>—</span> },
    {
      key: "source", label: "Source",
      render: (r) => (
        <span style={{
          fontSize: "10px", fontWeight: 700, padding: "3px 8px", borderRadius: "100px",
          background: r.source === "website" ? "rgba(0,194,255,0.1)" : "rgba(255,255,255,0.05)",
          color: r.source === "website" ? "#00C2FF" : "rgba(255,255,255,0.5)",
          border: `1px solid ${r.source === "website" ? "rgba(0,194,255,0.2)" : "rgba(255,255,255,0.08)"}`,
          letterSpacing: "0.5px",
        }}>
          {r.source === "website" ? "🌐 SELF" : (r.source || "—").toUpperCase()}
        </span>
      )
    },
    {
      ...
      render: (r) => r.notes
        ? <span style={{ fontSize: "12px", color: "rgba(255,255,255,0.4)", fontStyle: "italic" }}>
            "{r.notes.slice(0, 45)}{r.notes.length > 45 ? "…" : ""}"
          </span>
  ...
  return (
    <DashLayout title="LEADS" subtitle="Track enquiries and convert them to members"
            ...
            <form onSubmit={handleSave} style={ls.form}>
              <div style={ls.formGrid}>
                {[
                  ...
                  { key: "source",    label: "Source",     placeholder: "walk-in / referral" },
              ...
              <div style={ls.field}>
                ...
                <textarea placeholder="Any additional notes..." value={form.notes || ""} rows={3} style={ls.textarea}
        ...
        <div style={ls.overlay}>
          <div style={{ ...ls.modal, maxWidth: "560px" }}>
            ...
            <form onSubmit={handleConvert} style={ls.form}>
                  ...
                  {MEMBERSHIP_TYPES.map((t) => (
                    <button key={t} type="button"
                      onClick={() => setConvertForm((p) => ({
                        ...p,
                        membership_type: t,
                        end_date: calcEndDate(t, p.start_date),
                      }))}
                      style={{
                ...
                <div style={ls.field}>
                  ...
                  <input type="date" className="field-input" value={convertForm.start_date} style={ls.input}
                    onChange={(e) => setConvertForm((p) => ({
                      ...
                      end_date: calcEndDate(p.membership_type, e.target.value),
                    }))} />
                </div>
                <div style={ls.field}>
                  <label style={ls.label}>End Date (auto-calculated)</label>
              ...
              {/* Notes */}
              <div style={ls.field}>
                <label style={ls.label}>Admission Notes (optional)</label>
                <textarea placeholder="e.g. Walk-in converted after trial session. Interested in yoga classes."
              ...
              <div style={{
                ...
                borderRadius: "10px", padding: "0.875rem 1rem", fontSize: "12px",
                color: "rgba(255,255,255,0.45)", lineHeight: 1.6,
              }}>
                ℹ️ If this lead has no account yet, one will be created automatically using their email. They can reset their password on first login.
              </div>
