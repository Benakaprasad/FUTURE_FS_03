    ...
    <div style={{
      ...
      >
        <div style={{
          ...
          <span style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "0.9rem", letterSpacing: "3px", color: isExpired ? T.muted : tier.color }}>
            {tier.label}
          </span>
        </div>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: "11px", color: T.sub, marginTop: 2 }}>
            {isExpired
              ? "Expired — play the game again on the homepage"
              : reward.discount_used
              ? "Discount used · non-monetary perks still active"
      ...
      {isUrgent && (
        <div style={{ display: "flex", alignItems: "center", gap: 8, margin: "0 1.25rem 0.75rem", padding: "8px 12px", background: "rgba(255,26,26,0.06)", border: "1px solid rgba(255,26,26,0.2)", borderRadius: 8 }}>
          ...
          <span style={{ fontSize: "12px", color: T.muted }}>
            Discount expires in <strong style={{ color: T.red }}>{hoursLeft}h</strong> — activate membership to use it
          </span>
        </div>
      )}
        ...
        <div style={{ padding: "0 1.25rem", borderTop: `1px solid ${T.border}` }}>
          {perks.map((perk, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "10px 0", borderBottom: i < perks.length - 1 ? `1px solid ${T.border}` : "none", opacity: perk.used || isExpired ? 0.4 : 1 }}>
              <div style={{ width: 30, height: 30, flexShrink: 0, borderRadius: 8, background: `${perk.color}12`, border: `1px solid ${perk.color}22`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.8rem" }}>
                {perk.icon}
              </div>
              <div style={{ flex: 1 }}>
...
export default function CustomerDashboard() {
  ...
  const daysLeft = membership?.end_date
    ? Math.max(0, Math.ceil((new Date(membership.end_date) - new Date()) / 86400000)) : null;
  const totalDays = membership?.start_date && membership?.end_date
    ? Math.ceil((new Date(membership.end_date) - new Date(membership.start_date)) / 86400000) : null;
  ...
  const recentActivity = requests.slice(0, 4).map(r => {
    ...
    return { icon: info.icon, title: info.title, color: info.color,
  ...
  return (
    ...
    >
      <style>{`
        ...
        @media(max-width:380px){ .hero-right{display:none!important;} }
      `}</style>

      {loading ? <Loader /> : (
        <div>

          {/* ── Reward card — top of page, only shown if earned ── */}
          {reward && <RewardCard reward={reward} />}

          {/* ── Hero membership card ── */}
          {membership ? (
            <div className="hero-card" style={{ position: "relative", display: "flex", justifyContent: "space-between", alignItems: "center", background: T.glass, border: `1px solid ${planColor}25`, borderRadius: "18px", padding: "2rem", marginBottom: "1.5rem", overflow: "hidden", gap: "1.5rem", animation: "fadeUp 0.5s ease both" }}>
              ...
              <div style={{ position: "relative", zIndex: 1, flex: 1, minWidth: 260 }}>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: "10px", fontWeight: 800, letterSpacing: "2px", border: `1px solid ${planColor}40`, borderRadius: 100, padding: "5px 14px", marginBottom: "1rem", color: planColor, background: planColor + "12" }}>
                  ...
                  {membership.status === "active" ? "ACTIVE" : "EXPIRED"} · {membership.plan_type?.toUpperCase().replace("HALF_YEARLY","HALF-YEARLY")} PLAN
                </div>
                ...
                <div className="hero-btns" style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
                  <Link to="/dashboard/membership" style={{ ...Btn.primary, background: planColor, boxShadow: `0 4px 20px ${planColor}40` }}>Renew / Upgrade <ArrowRight size={13} style={{ marginLeft: 4, verticalAlign: "middle" }} /></Link>
